...
class UserHelper():
    ...
    def register(mail, password, username):
        result = {}
        if UserHelper.search_by_mail(mail) != None:
    ...
    def delete(id):
        delete_user = UserHelper.search_by_id(id)
    ...
    def update_mail(id, current_mail, new_mail, check_mail):
        if UserHelper.search_by_id(id).mail != current_mail:
            ...
            UserHelper.search_by_id(id).mail = new_mail
            db_orm.session.commit()
            result = 'success'   
        return result
    
    def update_password(id, current_pass, new_pass, check_pass):
        if check_password_hash(UserHelper.search_by_id(id).hashPASS, current_pass) == False:
            result = '現パスワードが誤っています'
        elif new_pass != check_pass:
            result = '新パスワードと確認用が一致しません'
        else:
            UserHelper.search_by_id(id).hashPASS = generate_password_hash(new_pass)
            db_orm.session.commit()
            result = 'success'
        return result

    def update_username(id, current_name, new_name, check_name):
        if UserHelper.search_by_id(id).username != current_name:
            result = '現ユーザーネームが誤っています'
        elif new_name != check_name:
            result = '新ユーザーネームと確認用が一致しません'
        else:
            UserHelper.search_by_id(id).username = new_name
            db_orm.session.commit()
            result = 'success'
        return result
